...
#define BC1e2_PIN_MASK  0x0C000000L
  ...
  while (exitBUS)
  {
    ...
    if (!busBit)
    {
      ...
      // Wait of 482ns 
        ...
        gpio_clr_mask(DIR_PIN_MASK); 
        while(((gpio_get_all() & BC1e2_PIN_MASK)>>BC1_PIN)==3); // wait while bc1 & bc2 are high... it's enough test BC1
...
void loadROM() {
  ...
  if (entry) {
   ...
   Serial.print("Rom lenght:");Serial.println(romLen,DEC);
  //verify
  //  slot=0;
    romLen=0;
    entry.seek(0);
